#pragma once

#include "AbstractFactory.h"
#include "HeapAndPriorityQueueTester.h"


class HeapAndPriorityQueueFactory : public AbstractFactory
{
public:
    std::unique_ptr<Solution> createSolution() { return std::make_unique<HeapAndPriorityQueue>(); }
    std::unique_ptr<Tester>   createTester() { return std::make_unique<HeapAndPriorityQueueTester>(); }

    class Builder : public AbstractBuilder
    {
    public:
        std::unique_ptr<Solution> buildSolution() override
        {
            auto solution = std::make_unique<HeapAndPriorityQueue>();
            // Apply other configurations...
            return solution;
        }

        std::unique_ptr<Tester> buildTester() override
        {
            auto tester = std::make_unique<HeapAndPriorityQueueTester>();
            tester->setVerbose(this->verbose);
            // Apply other configurations...
            return tester;
        }
    };
};
